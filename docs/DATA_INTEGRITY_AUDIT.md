...
The standalone repository test suite exercises success and failure states for the publication calendar, holidays, unchanged observations, future data, duplicates, stale sources, invalid denominators, malformed release calendars, atomic promotion, model calculation, formatting, accounting reconciliation, and page contracts. The current standalone run passes all 66 included tests. A new operating refresh is not promoted unless all required source freshness checks pass.
